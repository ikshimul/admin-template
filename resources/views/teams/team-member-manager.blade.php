<div>
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                ...
                <x-slot name="form">
                    ...
                    <!-- Member Email -->
                    <div class="col-md-6 sm:col-md-4 mb-3">
                        <x-label for="email" value="{{ __('Email') }}" />
                        <x-input id="email" type="email" class="" wire:model="addTeamMemberForm.email" />
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            ...
                            <div class="relative z-0 mt-1 border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer">
                                @foreach ($this->roles as $index => $role)
                                    <button type="button" class="btn btn-soft-light waves-effect waves-light relative tw-px-4 tw-py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 {{ $index > 0 ? 'border-t border-gray-200 dark:border-gray-700 focus:border-none rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="d-flex items-center justify-between">
                                <div class="col-md-6 d-flex items-center">
                                    ...
                                    <div class="ms-4 mt-1 dark:text-white">{{ $user->name }}</div>
                                </div>

                                <div class="col-md-6 d-flex items-center justify-between justify-content-end">
                                    <div class="d-flex flex-wrap gap-2 mb-0 my-n1 ">
                                        <!-- Manage Team Member Role -->
                                        @if (Gate::check('updateTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                            <button class="btn btn-sm btn-link btn-rounded waves-effect ms-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                        ...
                                        @if ($this->user->id === $user->id)
                                            <button class="btn btn-sm btn-link btn-rounded waves-effect cursor-pointer ms-6 text-sm text-danger" wire:click="$toggle('confirmingLeavingTeam')">
                                                {{ __('Leave') }}
                                            </button>

                                        <!-- Remove Team Member -->
                                        @elseif (Gate::check('removeTeamMember', $team))
                                            <button class="btn btn-sm btn-link btn-rounded waves-effect cursor-pointer ms-6 text-sm text-danger" wire:click="confirmTeamMemberRemoval('{{ $user->id }}')">
                                                {{ __('Remove') }}
                                            </button>
                                        @endif
                                    </div>
    ...
    <x-dialog-modal wire:model.live="currentlyManagingRole">
        ...
        <x-slot name="content">
            <div class="relative z-0 mt-1 border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer">
                @foreach ($this->roles as $index => $role)
                    <button type="button" class="btn btn-soft-light waves-effect waves-light relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 {{ $index > 0 ? 'border-t border-gray-200 dark:border-gray-700 focus:border-none rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
